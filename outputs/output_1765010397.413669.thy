...
| "apply_operations balance (op # ops) = 
    apply_operations (apply_operation balance op) ops"
...
lemma bank_account_bug:
  assumes "amount > balance"
  shows "apply_operation balance (Withdraw amount) < 0"
  using assms by simp
```